...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index: Any) -> Any:
        ...
        return {
            ...
            "encoder_mask": (encoder_input != self.pad_token).int().unsqueeze(0).unsqueeze(0),
            "decoder_mask": (decoder_input != self.pad_token).int().unsqueeze(0).unsqueeze(0) & causal_mask(decoder_input.size(0)),
            "label": label,
            "src_text": src_text,
            "tgt_text": tgt_text
        }
...
def causal_mask(size: int) -> torch.Tensor:
    mask = torch.tril(torch.ones((1, size, size)), diagonal=0).type(torch.int64)
    return mask
